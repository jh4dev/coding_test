        ...
        while(!queue.isEmpty()) {
        	...
        	for(int i = 0; i < vil.roadList.size(); i++) {
        		if(before == vil.roadList.get(i).end) {
        			...
        			tempRoad = vil.roadList.get(i);
        			if(vil.distance + tempRoad.cost <= K) {
        				
        				if(answerMap.get(tempRoad.end) == null 
        						|| vil.distance + tempRoad.cost < answerMap.get(tempRoad.end)) {
        					answerMap.put(tempRoad.end, vil.distance + tempRoad.cost);
        					queue.add(new Village(tempRoad.end, vil.distance + tempRoad.cost, roadMap.get(tempRoad.end)));
            				before = tempRoad.end;
        				} 
        			}
        		}
