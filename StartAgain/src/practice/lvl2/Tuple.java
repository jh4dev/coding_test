...
public class Tuple {
	...
	public static int[] solution(String s) {
        ...
        for(String t : s.split("/")) {
        	tempList.add(Arrays.asList(t.split(",")).stream().mapToInt(Integer::parseInt).toArray());
        }
        ...
        //첫번째 원소 셋팅
        answerList.add(tempList.get(0)[0]);
        
        Set<Integer> iSet;
        for(int i = 1; i < tempList.size(); i++) {
        	iSet = new HashSet<Integer>(answerList);
        	
        	for(int j = 0; j < tempList.get(i).length; j++) {
        		if(!iSet.contains(tempList.get(i)[j])) {
        			answerList.add(tempList.get(i)[j]);
        			break;
        		}
        	}
        }
