...
public class LionArrow {
	...
	public static void bt(int shoot, int n, int[] info, int target) {
		
		if(shoot == n) {
			
			if(Arrays.toString(lionTemp).equals("[1, 1, 0, 1, 0, 0, 0, 0, 0, 0, 0]")) {
				System.out.println("hi?");
			}
			if(Arrays.toString(lionTemp).equals("[1, 1, 2, 0, 1, 2, 2, 0, 0, 0, 0]")) {
				System.out.println("hi?");
			}
			...
			if(gap > 0 && gap >= maxGap) {
				...
				} else if(isCorrect(lionInfo, lionTemp)){
					maxGap = gap;
					lionInfo = lionTemp.clone();
				}
			}
		...
		//어피치보다 한 발 더 맞추거나, 안맞추거나
		int tempShoot = 0;
		while(shoot + tempShoot < n && tempShoot <= info[target]) {
			tempShoot++;
		}
		lionTemp[target] = tempShoot;
		bt(shoot + tempShoot, n, info, target+1);
		
		lionTemp[target] = 0;
		bt(shoot, n, info, target+1);
		
		
    }
	
	public static boolean isCorrect(int[] base, int[] now) {
		
		for(int i = base.length - 1; i >= 0; i--) {
			if(now[i] > base[i] && maxLastIndex < i) {
				maxLastIndex = i;
				return true;
			}
		}
