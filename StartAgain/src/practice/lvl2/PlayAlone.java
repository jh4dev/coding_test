...
public class PlayAlone {
	...
	public static int solution(int[] cards) {
        ...
        for(int i = 1; i <= cards.length; i++) {
        	boxMap.put(i, cards[i-1]);
        }
        
        for(int i = 1; i <= cards.length; i++) {
        	max1 = 0;
        	max2 = 0;
        	boolean[] visited = new boolean[boxMap.size()+1];
        	dfs(1, 0, i, boxMap, visited);
        	
//        	if()
        	
        	for(int j = 1; j < cards.length; j++) {
        		dfs(2, 0, j, boxMap, visited);
        		if(max1 * max2 > max) {
        			max = max1 * max2;
        		}
        	}
	...
	public static void dfs(int group, int cnt, int next, Map<Integer, Integer> boxMap, boolean[] visited) {
		...
		dfs(group, cnt+1, boxMap.get(next), boxMap, visited);
	}
}
