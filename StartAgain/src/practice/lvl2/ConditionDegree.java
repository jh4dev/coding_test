 ...
 * */
public class ConditionDegree {

	static int maxClear = 0;
	...
	public static void dfs(int k, int[][] dungeons, int clearCnt) {
		for(int i = 0; i < dungeons.length; i++) {
			
			if(!check[i] && dungeons[i][0] <= k && k - dungeons[i][1] >= 0) {
				check[i] =true;
				dfs(k - dungeons[i][1], dungeons, clearCnt+1);
				check[i] =false;
			}
		}
		if(clearCnt > maxClear) maxClear = clearCnt;
	}
