 ...
 * */
public class SeperateNumberCard_91Score {
	...
	public static int solution(int[] arrayA, int[] arrayB) {
       ...
       //가장 큰 수와 가장 작은 수의 최대 공약수의 약수들로 확인
       int gcdA = getGCD(arrA[0], arrA[arrA.length-1]);
       int gcdB = getGCD(arrB[0], arrB[arrB.length-1]);
       
       Stack<Integer> as = new Stack<Integer>();
       Stack<Integer> bs = new Stack<Integer>();
       as.addAll(getDivisor(gcdA));
       bs.addAll(getDivisor(gcdB));
       
       for(int i = 0; i < arrA.length; i++) {
    	   
    	   if(!bs.isEmpty() && ((arrB[i] % bs.peek() != 0) || arrA[i] % bs.peek() == 0)) {
    		   bs.pop();
    	   }
    	   if(!as.isEmpty() && ((arrA[i] % as.peek() != 0) || arrB[i] % as.peek() == 0)) {
    		   as.pop();
    	   }
       ...
       if(as.isEmpty()) return bs.peek();
       else if(bs.isEmpty()) return as.peek();
       else return Math.max(as.peek(), bs.peek());
    }
