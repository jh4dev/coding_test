 ...
 * 철수는 롤케이크를 두 조각으로 잘라서 동생과 한 조각씩 나눠 먹으려고 합니다.
	...
	만약 세 번째 토핑(1)과 네 번째 토핑(3) 사이를 자르면 롤케이크의 토핑은 [1, 2, 1], [3, 1, 4, 1, 2]로 나뉘게 됩니다. 철수가 [1, 2, 1]이 놓인 조각을, 
	동생이 [3, 1, 4, 1, 2]가 놓인 조각을 먹게 되면 철수는 두 가지 토핑(1, 2)을 맛볼 수 있지만, 동생은 네 가지 토핑(1, 2, 3, 4)을 맛볼 수 있으므로, 이는 공평하게 나누어진 것이 아닙니다. 
	만약 롤케이크의 네 번째 토핑(3)과 다섯 번째 토핑(1) 사이를 자르면 [1, 2, 1, 3], [1, 4, 1, 2]로 나뉘게 됩니다. 이 경우 철수는 세 가지 토핑(1, 2, 3)을, 
	동생도 세 가지 토핑(1, 2, 4)을 맛볼 수 있으므로, 이는 공평하게 나누어진 것입니다. 공평하게 롤케이크를 자르는 방법은 여러가지 일 수 있습니다. 
...
public class RollcakeCutting {
	...
	public static int solution(int[] topping) {
        ...
        yMap.put(topping[0], 1);
        
        for(int i = 1; i < topping.length; i++) {
        	oMap.put(topping[i], oMap.getOrDefault(topping[i], 0) + 1);
        }
        
        
        for(int i = 1; i < topping.length-1; i++) {
        	yMap.put(topping[i], yMap.getOrDefault(topping[i], 0) + 1);
        	if(oMap.get(topping[i]) == 1) {
        		oMap.remove(topping[i]);
        	} else if(oMap.get(topping[i]) == null) {
        		continue;
        	} else {
        		oMap.put(topping[i], oMap.get(topping[i]) - 1);
        	}
        	
        	if(yMap.size() == oMap.size()) answer++;
        }
