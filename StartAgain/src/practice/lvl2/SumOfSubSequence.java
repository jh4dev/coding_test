...
//        	}
        	...
        	if(tempSum > k) {
        		...
        		tempSum += sequence[endIdx];
        	}
        	
        	if(tempSum == k) {
        		if(endIdx - startIdx + 1 < answer[1] - answer[0] + 1) {
        			answer[0] = startIdx;
        			answer[1] = endIdx;
        		}
        		//sequence의 원소는 1부터 이므로, 뒤에 0이 나와 같은 합이 발생할 수 없음
        		//따라서 startIdx 진행
        		tempSum -= sequence[startIdx];
        		startIdx += 1;
        	}
