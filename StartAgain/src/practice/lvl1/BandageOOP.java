...
public class BandageOOP {
	...
	public static int solution(int[] bandage, int health, int[][] attacks) {
        ...
        while(sec <= attacks[attacks.length - 1][0]) {
        	...
        	if(attacks[attackIdx][0] == sec) {
        		player.beAttacked(attacks[attackIdx][1]);
        		attackIdx++;
				//사망 체크
        		if(!player.isAlive()) {
        			return -1;
	...
	public static class Player {
		...
		public Player(int[] bandage, int health) {
		...
		public boolean isAlive() {
			return nowHp <= 0 ? false : true; 
		}
		
		public void useBandage() {
			
			this.nowHp = this.nowHp + this.bx;
			this.bcnt++;
			...
			this.nowHp = this.nowHp > this.hp ? this.hp : this.nowHp;
		}
		...
		public String toString() {
			return "Player [hp=" + hp + ", nowHp=" + nowHp + ", bt=" + bt + ", bx=" + bx + ", by=" + by + ", bcnt="
					+ bcnt + "]";
		}
	}
}
